...
fn test_memory_insert_and_retrieve() {
    ...
    store.insert("api_key_location", "Stored in 1Password vault 'Engineering'").unwrap();
...
fn test_memory_fts_search_relevance() {
    let store = MemoryStore::in_memory().unwrap();
    store.insert("rust_framework", "Ferroclaw is a Rust-based AI agent framework for security-first operation").unwrap();
    store.insert("python_tool", "dietmcp is a Python tool for compressing MCP schemas to reduce token usage").unwrap();
    store.insert("js_project", "NanoClaw is a lightweight JavaScript agent that uses Anthropic APIs").unwrap();
    store.insert("security_notes", "The capability system uses Ed25519 signed manifests and hash-chained audit logs").unwrap();
...
fn test_memory_search_limit() {
    let store = MemoryStore::in_memory().unwrap();
    for i in 0..20 {
        store.insert(&format!("item_{i}"), &format!("test data item number {i}")).unwrap();
    }
...
fn test_conversation_persistence() {
    ...
    store.save_conversation(session, "user", "What is Ferroclaw?").unwrap();
    store.save_conversation(session, "assistant", "Ferroclaw is a security-first Rust agent.").unwrap();
    store.save_conversation(session, "user", "How does it handle MCP?").unwrap();
    store.save_conversation(session, "assistant", "It uses native DietMCP compression.").unwrap();
...
fn test_conversation_isolation_between_sessions() {
    ...
    store.save_conversation("sess_a", "user", "Message for A").unwrap();
    store.save_conversation("sess_b", "user", "Message for B").unwrap();
    store.save_conversation("sess_a", "assistant", "Reply in A").unwrap();
...
fn test_memory_handles_unicode() {
    let store = MemoryStore::in_memory().unwrap();
    store.insert("chinese", "Ferroclaw 是一个安全优先的 AI 代理框架").unwrap();
    store.insert("emoji", "Status: running 🦀🔒").unwrap();
    store.insert("arabic", "فيروكلو هو إطار عمل وكيل الذكاء الاصطناعي").unwrap();
...
fn test_memory_concurrent_safe() {
    ...
    let store = MemoryStore::in_memory().unwrap();
    for i in 0..100 {
        store.insert(&format!("key_{i}"), &format!("value_{i}")).unwrap();
    }
    let all = store.list_all().unwrap();
